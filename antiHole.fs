FeatureScript 2837;
// Anti-Hole Feature - Creates additive geometry instead of subtractive holes
// Based on the Onshape Standard Library hole.fs
// Modified to create new bodies or add to existing bodies
...
/**
 * Defines how anti-hole geometry should be generated.
 * @value NEW_BODY : Create anti-hole geometry as new separate body.
 * @value ADD : Add anti-hole geometry to existing bodies via boolean union.
 */
export enum AntiHoleGenerationType
{
...
/**
* Creates anti-holes (additive geometry) of specific dimensions and style, based either on standard
 * hole size, or by user-defined values. Each anti-hole's position and orientation
 * are specified using sketch points. Anti-holes create new bodies or add to existing bodies.
 */
annotation { "Feature Type Name" : "Anti-Hole", "Editing Logic Function" : "antiHoleEditLogic", "Feature Name Template": "#featureName" }
export const antiHole = defineFeature(function(context is Context, id is Id, definition is map)
    precondition
    {
        annotation { "Name" : "Generation type", "UIHint" : ["REMEMBER_PREVIOUS_VALUE", "HORIZONTAL_ENUM", "SHOW_LABEL"] }
        definition.generationType is AntiHoleGenerationType;
        ...
        annotation { "Name" : "Sketch points to place anti-holes",
                    "Filter" : EntityType.VERTEX && SketchObject.YES && ModifiableEntityOnly.YES || BodyType.MATE_CONNECTOR,
                    "UIHint" : UIHint.INITIAL_FOCUS }
        definition.locations is Query;

        if (definition.generationType == AntiHoleGenerationType.ADD)
        {
            annotation { "Name" : "Bodies to merge with",
    ...
    }
    {
        ...
        // -- If any feature status is set above this line, `produceAntiHoles` will display the anti-hole tools as error entities --

        // ------------- Perform the operation -------------
        produceAntiHoles(context, id, definition, locations);
...
/**
 * Main function for producing anti-hole geometry.
 * Creates additive bodies in the shape of holes instead of cutting them.
 * 
 ...
 */
function produceAntiHoles(context is Context, topLevelId is Id, definition is map, locations is array)
{
    // For anti-holes, we don't need to check scope if creating new bodies
    // But we do need to check if adding to existing bodies
    if (definition.generationType == AntiHoleGenerationType.ADD)
    {
        definition.scopeSize = 0.1 * meter; // Set a default so that if we fail early, we can still generate the error entities
    ...
    {
        // For new body creation, set a default scope size
        definition.scopeSize = 0.1 * meter;
    }
    ...
    // Use the modern opHole-based pipeline to create the geometry
    produceAntiHolesUsingOpHole(context, topLevelId, definition, locations);
}

/**
 * Creates anti-hole geometry using the opHole operation.
 * This function builds the hole profiles and creates solid bodies,
 * then either leaves them as new bodies or merges them with existing bodies.
 * 
 * @param context - The current modeling context
 * @param topLevelId - The ID for this feature operation
 * @param definition - Feature definition map containing all parameters
 * @param locations - Array of location queries where anti-holes should be placed
 */
function produceAntiHolesUsingOpHole(context is Context, topLevelId is Id, definition is map, locations is array)
{
    ...
    // At this point, opHole has created solid bodies in the shape of holes
    // Now we need to either keep them as new bodies or merge them with existing bodies
    const antiHoleToolsQuery = qBodyType(qCreatedBy(opHoleId, EntityType.BODY), BodyType.SOLID);
    
    if (definition.generationType == AntiHoleGenerationType.ADD && !isQueryEmpty(context, definition.scope))
    {
    ...
    for (var i = 0; i < numberOfAntiHoles; i += 1)
    {
        ...
        // Create attributes for the anti-hole geometry
...
/**
 * @internal
 * Editing logic for anti-hole feature.
 */
export function antiHoleEditLogic(context is Context, id is Id, oldDefinition is map, definition is map,
    isCreating is boolean, specifiedParameters is map, hiddenBodies is Query) returns map
{
    ...
    if (isCreating)
    {
        ...
        // Set default generation type for new features
        if (definition.generationType == undefined)
        {
            definition.generationType = AntiHoleGenerationType.NEW_BODY;
        }
    }
    ...
    // Only run scope heuristics if ADD mode is selected
    if (definition.generationType == AntiHoleGenerationType.ADD)
    {
    ...
    {
        // For NEW_BODY mode, ensure scope is empty
        if (!specifiedParameters.scope)
        {
            definition.scope = qNothing();
        }
    }
