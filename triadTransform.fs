 ...
 */
export function triadTransformManipulatorChange(context is Context, output is map, input is map) returns map
{
    if (input[TRIAD_MANIPULATOR] is map)
    {
        const triadTransform = input[TRIAD_MANIPULATOR].transform;
        const rotation = transpose(triadTransform.linear);
        const angles = matrixToXYZAngles(rotation);
        output.dx = triadTransform.translation[0];
        output.dy = triadTransform.translation[1];
        output.dz = triadTransform.translation[2];
        output.rx = angles[0];
        output.ry = angles[1];
        output.rz = angles[2];
    }
