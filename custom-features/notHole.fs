...
icon::import(path : "87c7ef7de441b9e312385c57", version : "3281a78d19113a2999388eda");
...
/** @internal */
export enum HoleBooleanOperation
{
    annotation { "Name" : "New" }
    NEW,
    annotation { "Name" : "Add" }
 ...
 */
annotation { "Feature Type Name" : "Not hole", "Editing Logic Function" : "holeEditLogic", "Feature Name Template": "#featureName", "Icon" : icon::BLOB_DATA }
export const notHole = defineSheetMetalFeature(function(context is Context, id is Id, definition is map)
    precondition
    {
        ...
        annotation { "Name" : "Operation", "UIHint" : ["REMEMBER_PREVIOUS_VALUE", "HORIZONTAL_ENUM"] }
        definition.booleanOperation is HoleBooleanOperation;

        if (definition.isV2)
        {
            annotation { "Name" : "Units", "UIHint" : ["REMEMBER_PREVIOUS_VALUE", "HORIZONTAL_ENUM"] }
        ...
        if (definition.booleanOperation == HoleBooleanOperation.REMOVE || definition.booleanOperation == HoleBooleanOperation.ADD)
        {
        ...
        if (definition.isV2)
        {
            if (definition.unitsSystem == UnitsSystem.INCH)
            {
                ...
                {
                    annotation { "Name" : "Standard", "Lookup Table" : ANSI_HoleTableEx, "UIHint" : ["REMEMBER_PREVIOUS_VALUE"] }
                    definition.ansiHoleTableEx is LookupTablePath;
                }
                else
                {
                    annotation { "Name" : "Standard", "Lookup Table" : ANSI_HoleTable, "UIHint" : ["REMEMBER_PREVIOUS_VALUE"] }
            ...
            else if (definition.unitsSystem == UnitsSystem.METRIC)
            {
                if (definition.style == HoleStyle.SIMPLE)
                {
                    annotation { "Name" : "Standard", "Lookup Table" : ISO_HoleTableEx, "UIHint" : ["REMEMBER_PREVIOUS_VALUE"] }
                    definition.isoHoleTableEx is LookupTablePath;
                }
                else
                {
                    annotation { "Name" : "Standard", "Lookup Table" : ISO_HoleTable, "UIHint" : ["REMEMBER_PREVIOUS_VALUE"] }
        ...
        if (definition.threadStandard != ThreadStandard.UNSET && definition.isV2)
        {
            ...
            if (definition.showThreadClassV2)
            {
                annotation { "Group Name" : "Thread class", "Driving Parameter" : "showThreadClassV2", "Collapsed By Default" : false }
                {
                    ...
                    {
                        annotation { "Name" : "Thread class", "Lookup Table" : ANSI_ThreadClassHoleTable, "UIHint" : ["REMEMBER_PREVIOUS_VALUE"] }
                        definition.ansiThreadClassV2 is LookupTablePath;
                    }
                    else
                    {
                        annotation { "Name" : "Thread class", "Lookup Table" : ISO_ThreadClassHoleTable, "UIHint" : ["REMEMBER_PREVIOUS_VALUE"] }
        ...
        if ((!definition.isV2 && (definition.endStyle != HoleEndStyle.THROUGH || (definition.booleanOperation == HoleBooleanOperation.NEW || definition.booleanOperation == HoleBooleanOperation.ADD))) ||
            (definition.isV2 && (definition.endStyleV2 != HoleEndStyleV2.THROUGH || (definition.booleanOperation == HoleBooleanOperation.NEW || definition.booleanOperation == HoleBooleanOperation.ADD))))
        {
    ...
    }
    {
        ...
        if (definition.booleanOperation == HoleBooleanOperation.NEW)
        {
            definition.scope = qEverything(EntityType.BODY)->qBodyType(BodyType.SOLID);
        }
...
{
    const shouldThrowScopeErrors = throwScopeErrors(definition);

    // ------------- Check scope ---------------
    const opScope = definition.scope == undefined ? qAllModifiableSolidBodies() : definition.scope;
    ...
    const scopeTest = evaluateQuery(context, opScope);
    if (size(scopeTest) == 0 && shouldThrowScopeErrors)
    {
    ...
    {
        if (!shouldThrowScopeErrors || isAtVersionOrLater(context, FeatureScriptVersionNumber.V1285_HOLE_SCOPE_DEFAULT))
        {
            // Give some default so that we do not fail during error entity generation
            definition.scopeSize = 0.1 * meter;
        }
        if (shouldThrowScopeErrors)
        {
...
function createPositiveHoleBodies(definition is map) returns boolean
{
    return definition.booleanOperation == HoleBooleanOperation.NEW || definition.booleanOperation == HoleBooleanOperation.ADD;
}

function addPositiveHoleBodiesToScope(definition is map) returns boolean
{
    return definition.booleanOperation == HoleBooleanOperation.ADD;
}

function throwScopeErrors(definition is map) returns boolean
{
    return definition.booleanOperation == HoleBooleanOperation.REMOVE;
}
...
function produceHolesUsingOpHole(context is Context, topLevelId is Id, definition is map, locations is array)
{
    ...
    const fallbackOpHoleId = opHoleId + "standaloneBodies";
    var activeOpHoleId = opHoleId;
    const usePositiveBodies = createPositiveHoleBodies(definition);
    // Copy the feature definition so NEW-mode generation tweaks (e.g. BLIND fallback for THROUGH)
    // do not mutate the original definition used later for hole attributes/callouts.
    var opHoleDefinition = mergeMaps(definition, {});
    if (usePositiveBodies && evaluateQueryCount(context, qAllModifiableSolidBodies()) == 0)
    {
        // In an empty part studio, avoid target-referenced hole construction and create standalone bodies from sketch points.
        opHoleDefinition.startStyle = HoleStartStyle.SKETCH;
        opHoleDefinition.scope = qNothing();
    }
    if (usePositiveBodies && opHoleDefinition.endStyle == HoleEndStyle.THROUGH)
    {
        // NEW-mode THROUGH should generate standalone geometry like BLIND (depth-driven), but keep THROUGH semantics for attribution.
        opHoleDefinition.endStyle = HoleEndStyle.BLIND;
    }
    var opHoleInfo;
    try
    {
        if (usePositiveBodies)
        {
            opHoleInfo = try silent(buildOpHoleDefinitionAndCallOpHole(context, topLevelId, opHoleId, opHoleDefinition, locations, {
                        // REMOVE keeps legacy subtractive behavior, NEW/ADD keeps generated tool bodies and merges explicitly.
            ...
            if (opHoleInfo == undefined)
            {
                // Stay silent in NEW/ADD and immediately try standalone fallback instead of throwing HOLE_NO_HITS.
                clearFeatureStatus(context, topLevelId, {});
                var fallbackDefinition = opHoleDefinition;
                fallbackDefinition.startStyle = HoleStartStyle.SKETCH;
                fallbackDefinition.scope = qNothing();
                opHoleInfo = try silent(buildOpHoleDefinitionAndCallOpHole(context, topLevelId, fallbackOpHoleId, fallbackDefinition, locations, {
                            "subtractFromTargets" : false,
                            "targetsToExcludeFromSubtraction" : qNothing(),
                            "useSMDefinitionTopologyId" : false,
                            "keepTools" : true
                        }));
                if (opHoleInfo == undefined)
                {
                ...
                activeOpHoleId = fallbackOpHoleId;
            }
        }
        else
        {
            opHoleInfo = buildOpHoleDefinitionAndCallOpHole(context, topLevelId, opHoleId, opHoleDefinition, locations, {
                        // REMOVE keeps legacy subtractive behavior, NEW keeps generated tool bodies and merges them explicitly.
    ...
    {
        if (usePositiveBodies && error == ErrorStringEnum.HOLE_NO_HITS)
        {
            // For NEW/ADD modes, allow standalone positive hole bodies even when axis direction misses all parts.
            clearFeatureStatus(context, topLevelId, {});
            var fallbackDefinition = opHoleDefinition;
            fallbackDefinition.startStyle = HoleStartStyle.SKETCH;
            fallbackDefinition.scope = qNothing();
    ...
    if (addPositiveHoleBodiesToScope(definition) && getFeatureInfo(context, topLevelId) == ErrorStringEnum.HOLE_NO_HITS)
    {
        clearFeatureStatus(context, topLevelId, {});
    }
    ...
    if (sheetMetalTargetInfo.hasSheetMetalTargets)
    {
        ...
        wallToCuttingToolBodyIds = handleSheetMetalCutAndAttribution(context, topLevelId, activeOpHoleId, definition, opHoleInfo.returnMapPerHole,
            sheetMetalTargetInfo, locations, successfulHoles);
    }
    if (usePositiveBodies ||
        sheetMetalTargetInfo.hasNonSheetMetalTargets ||
        isAtVersionOrLater(context, FeatureScriptVersionNumber.V2209_COUNTER_HOLES_IN_SHEET_METAL))
    {
        ...
        for (var i = 0; i < nHoles; i += 1)
        {
            ...
            const instanceProducedFaces = createAttributesFromQuery(context, topLevelId, activeOpHoleId, definition,
            ...
            else if (usePositiveBodies)
            {
                // In NEW mode, opHole may succeed without target-face attribution. Count those successful tool-body
                // instances so we don't incorrectly surface HOLE_NO_HITS.
                successfulHoles[][i] = true;
            }
        }
    }
    ...
    if (usePositiveBodies)
    {
        const holeToolBodies = qBodyType(qCreatedBy(activeOpHoleId, EntityType.BODY), BodyType.SOLID);
        if (addPositiveHoleBodiesToScope(definition))
        {
            // ADD mode unions positive hole bodies into merge scope. For UNION, opBoolean uses tools only.
            if (!isQueryEmpty(context, holeToolBodies))
            {
                opBoolean(context, topLevelId + "nonSheetMetalHoleAdd", {
                            "tools" : qUnion([definition.scope, holeToolBodies]),
        ...
        {
            // NEW mode keeps standalone positive bodies and only unions tool fragments together when needed.
            if (evaluateQueryCount(context, holeToolBodies) > 1)
            {
                callSubfeatureAndProcessStatus(topLevelId, booleanBodies, context, topLevelId + "nonSheetMetalHoleMerge", {
                            "tools" : holeToolBodies,
                            "operationType" : BooleanOperationType.UNION
                        }, {
                            "featureParameterMap" : { "tools" : "locations" },
...
function buildOpHoleDefinitionAndCallOpHole(context is Context, topLevelId is Id, opHoleId, definition is map, locations is array, opHoleOverrides is map) returns map
{
    ...
    const opHoleTargets = definition.scope == undefined ? qAllModifiableSolidBodies() : definition.scope;
    var opHoleDefinition = mergeMaps({
                "holeDefinition" : holeDef,
                "axes" : axes,
                "identities" : locations,
                "targets" : opHoleTargets,
                "endBoundEntity" : endBound
            }, opHoleOverrides);
    if (opHoleDefinition.targets == undefined)
    {
        opHoleDefinition.targets = qAllModifiableSolidBodies();
    }
    const returnMapPerHole = callSubfeatureAndProcessStatus(topLevelId, opHole, context, opHoleId, opHoleDefinition, {
                "propagateErrorDisplay" : true
            });
    ...
    try
    {
        ...
        if (!isQueryEmpty(context, holeToolBodiesQ))
        {
            ...
            callSubfeatureAndProcessStatus(topLevelId, booleanBodies, context, enableCounterHolesInSM ? booleanId + "old" : booleanId, {
                        ...
                        "keepTools" : usePositiveBodies || showUnsupportedCounterHoleEntities,
                        "operationType" : usePositiveBodies ? BooleanOperationType.UNION : BooleanOperationType.SUBTRACTION
                    });
    ...
    if (definition.cutOption == HoleCutOption.CUT_ALL && result.numSuccess > 0)
    {
        ...
        try
        {
            booleanBodies(context, booleanId, { "targets" : definition.scope,
                        ...
                        "operationType" : createPositiveHoleBodies(definition) ? BooleanOperationType.UNION : BooleanOperationType.SUBTRACTION
                    });
        }
...
function getSheetMetalTargetInfo(context is Context, featureDefinition is map)
{
    ...
    const nonSheetMetalTargets = separatedQueries.nonSheetMetalQueries;
    const hasSheetMetalTargets = !isQueryEmpty(context, sheetMetalTargets);

    return {
            "hasSheetMetalTargets" : hasSheetMetalTargets,
            "hasNonSheetMetalTargets" : !isQueryEmpty(context, nonSheetMetalTargets),
            "nonSheetMetalTargets" : makeRobustQuery(context, nonSheetMetalTargets),
...
function cutHole(context is Context, id is Id, holeDefinition is map, holeNumber is number, startDistances is map, cSys is CoordSystem) returns map
{
    ...
    if (isAtVersionOrLater(context, FeatureScriptVersionNumber.V429_HOLE_SAFE_SKETCH_CLEANUP))
    {
        try
        {
            spinCut(context, id, sketchQuery, axisQuery, holeDefinition.scope, !doCut, createPositiveHoleBodies(holeDefinition));
...
function spinCut(context is Context, id is Id, sketchQuery is Query, axisQuery is Query, scopeQuery is Query, makeNew is boolean, createPositiveBodies is boolean)
{
    revolve(context, id, {
                "bodyType" : ExtendedToolBodyType.SOLID,
                "operationType" : makeNew ? NewBodyOperationType.NEW : (createPositiveBodies ? NewBodyOperationType.ADD : NewBodyOperationType.REMOVE),
    ...
    holeIdentity is Query, singleHoleReturnValue is map, holeIndex is number, userDefinedHoleDepth, wallToCuttingToolBodyIds) returns boolean
{
    ...
    for (var target in targetBodies)
    {
        ...
        var finalPositionReferenceInfo = singleHoleReturnValue.positionReferenceInfo[finalPositionReference];
        if (finalPositionReferenceInfo == undefined && createPositiveHoleBodies(featureDefinition))
        {
            // Standalone NEW-mode hole tools may not report target-based position reference info.
            finalPositionReferenceInfo = { "referenceRootStart" : 0 * meter, "referenceRootEnd" : 0 * meter };
        }
        ...
        if (depthExtremes == undefined && createPositiveHoleBodies(featureDefinition))
        {
            // In NEW mode, target faces belong to created tool bodies and may not be keyed in
            // `targetToDepthExtremes`; use the per-hole depth extremes if available.
            for (var depthExtremesEntry in singleHoleReturnValue.targetToDepthExtremes)
            {
                depthExtremes = depthExtremesEntry.value;
                break;
            }
            if (depthExtremes == undefined)
            {
                // In empty studios there are no target intersections; use axis-origin defaults so hole attributes and
                // cosmetic thread attributes can still be written onto the generated positive hole faces.
                depthExtremes = {
                        "firstEntrance" : 0 * meter,
                        "fullEntrance" : 0 * meter,
                        "firstExit" : 0 * meter,
                        "fullExit" : 0 * meter
                    };
                ...
                {
                    attributedFaces = append(attributedFaces, faceAndSectionFaceType.key);
                }
                if (attributedFaces != [])
                {
                    const actualHoleDepth = try(computeActualHoleDepth(context, qUnion(attributedFaces)));
                    if (actualHoleDepth != undefined)
                    {
                        depthExtremes.firstExit = actualHoleDepth;
                        depthExtremes.fullExit = actualHoleDepth;
                    }
                }
            }
        }
        ...
        if (createPositiveHoleBodies(featureDefinition) && featureDefinition.endStyle == HoleEndStyle.THROUGH)
        {
            // Geometry is produced with BLIND depth in NEW mode, but callout semantics should remain THROUGH.
            featureDefinitionForAttribute.endStyle = HoleEndStyle.THROUGH;
        }

        // Adjust `partialThrough`
        const newThroughAll = createPositiveHoleBodies(featureDefinition) && featureDefinition.endStyle == HoleEndStyle.THROUGH;
        if (featureDefinitionForAttribute.endStyle == HoleEndStyle.THROUGH)
        {
            if (newThroughAll)
            {
                // NEW-mode THROUGH uses blind-style generation depth; keep callouts as full through.
                featureDefinitionForAttribute.partialThrough = false;
            }
            else if (faceTypes[HoleFaceType.CAP] == true)
            {
        ...
        for (var faceAndSectionFaceType in faceToSectionFaceType)
        {
            if (newThroughAll && faceAndSectionFaceType.value == HoleSectionFaceType.BLIND_TIP_FACE)
            {
                // NEW-mode THROUGH is generated as BLIND geometry; don't tag the synthetic tip face as a blind section.
                continue;
            }
