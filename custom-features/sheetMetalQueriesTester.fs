...
// Note: These paths assume the files are in the custom-features directory
// In Onshape, you would use the full document path
...
export const sheetMetalQueryTester = defineFeature(function(context is Context, id is Id, definition is map)
    ...
    }
    {
        ...
        catch (error)
        {
            ...
            println("Error executing query operation: " ~ error);
            reportFeatureWarning(context, id, "Query operation failed. See console for details.");
        }
    });
