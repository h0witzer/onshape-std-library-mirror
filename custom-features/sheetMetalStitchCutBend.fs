    ...
    definition is map, defaultRadius, defaultKFactor, bendReliefParams, sheetMetalThickness, modelParameters) returns Query
{
    ...
    const path = try (constructPath(context, orderedEdgeQuery));
    if (path == undefined)
    {
...
}
{
    ...
    const computedAngle = try (bendAngle(context, id + "angle", jointEdge, radius));
    ...
    existingAttribute is SMAttribute, defaultRadius, bendReliefParams, sheetMetalThickness, modelParameters, bendReliefSubsegmentSize)
{
    ...
    const reliefDepthAdjustment = ((depthScale-1) * sheetMetalThickness) + (.5 * widthScale * sheetMetalThickness);
    ...
    // Following Sheet Metal Tab pattern (sheetMetalTab.fs line 533)
    if (size(cylinderBodies) > 0)
    {
        try silent(opDeleteBodies(context, id + "deleteCylinders", {
            "entities" : qUnion(cylinderBodies)
        }));
    }
}
    ...
    totalLength is ValueWithUnits, domains is array) returns Query
{
    ...
    const orderedPath = try (constructPath(context, qUnion(edges)));
