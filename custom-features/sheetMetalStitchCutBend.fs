...
export const sheetMetalStitchCutBend = defineSheetMetalFeature(function(context is Context, id is Id, definition is map)
    ...
    }
    {
        ...
        // NOTE: Debug output shows all split vertexes are NOT_A_CORNER until updateSheetMetalGeometry processes them.
        // The sheet metal system should automatically generate bend reliefs at bend ends based on:
        // 1. The bend edge attributes (which we've set)
        // 2. The model's default bend relief parameters (defaultBendReliefStyle, etc.)
        // Manual corner attribution before updateSheetMetalGeometry doesn't work because corners don't exist yet.
        println("\n====== STEP 5: Bend Relief (Auto-Generated by Sheet Metal System) ======");
        println("Bridge segment count: " ~ bridgeSegmentCount);
        println("Stitch count: " ~ stitchCount);
        println("Bend reliefs should be automatically generated at bend ends by updateSheetMetalGeometry");
        println("based on model parameters: style=" ~ modelAttribute.defaultBendReliefStyle);
        
        // DISABLED: Manual attribution doesn't work because vertexes are NOT_A_CORNER before geometry update
        // if (bridgeSegmentCount > 0 && stitchCount > 0)
        // {
        ...
        // Bend reliefs should be generated automatically at bend ends
