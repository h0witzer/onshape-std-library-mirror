...
export function instantiate(context is Context, instantiator is Instantiator)
{
    ...
    for (var entry in instantiator[].buildAndConfigurationToInstances)
    {
        ...
        for (var tolerantConfigurationInstances in tolerantConfigurations)
        {
            ...
            const derivedResult = derive(context, derivedId, build, {
                ...
                "includeVariables" : instantiator[].includeVariables,
                "variablePrefix" : instantiator[].variablePrefix,
                "variableSuffix" : instantiator[].variableSuffix
                });
            if (derivedResult == {})
                continue;
    ...
    try silent { opDeleteBodies(context, instantiator[].id + "delete", { "entities" : qCreatedBy(instantiator[].id + "derived", EntityType.BODY) }); }
}
...
function derive(context is Context, id is Id, buildFunction is function, options is map) returns map
{
    ...
    {
        for (var variable in getAllVariables(otherContext))
        {
            setVariable(context, options.variablePrefix ~ variable.key ~ options.variableSuffix, variable.value);
        }
    }
