    ...
    }
    {
        ...
        if (definition.skipInstances)
        {
            ...
            const instanceToIndex = function(instance)
                {
                    return definition.isCentered ? instance.index + definition.instanceCount - 1 : instance.index;
                };
...
export function circularPatternPointChange(context is Context, definition is map, newManipulators is map) returns map
{
    const indexToInstance = function(index)
        {
            return { "index" : definition.isCentered ? index - definition.instanceCount + 1 : index };
        };
