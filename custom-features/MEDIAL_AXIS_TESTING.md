...
6. Set sample density to 10 (default, good for simple shapes)
...
5. Set sample density to 10 (default)
6. Run the feature
...
**Possible Causes**:
- Sample density too low for the shape complexity
- Boundary has high curvature regions that need more samples

**Solutions**:
1. Increase sample density gradually (try 15, 20, then 30)
2. For curved boundaries, may need sample density of 30-50
3. Check edge lengths in console output
4. Note: Higher sample density increases computation time
...
- **Simple shapes** (rectangles, circles with default settings): < 1 second
- **Medium shapes** (polygons with 20-50 segments): 1-3 seconds
- **Complex shapes** (curved boundaries, sample density 30-50): 3-10 seconds
- **Very high detail** (sample density > 50): May be slow, use sparingly

### Performance Notes
The current implementation uses optimized collision testing:
- Only adjacent segments are always tested (O(n))
- For shapes with >20 segments, tests a small window of nearby segments
- Much faster than the original O(n²) all-pairs approach
- Numerical collision solver uses 5 iterations (reduced from 20)
...
1. **Adjacent-Only Collision Testing**: Only tests adjacent segments by default for maximum speed
2. **Limited Window Testing**: For complex shapes, tests a small window of nearby segments
3. **Reduced Numerical Iterations**: Uses 5 iterations instead of 20 for collision solving
...
7. **Node Duplication**: Creates separate nodes for each edge instead of sharing
