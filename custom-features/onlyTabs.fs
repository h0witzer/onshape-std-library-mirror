...
import(path : "onshape/std/extendsheetshapetype.gen.fs", version : "2856.0");
import(path : "onshape/std/edgetopology.gen.fs", version : "2856.0");
...
import(path : "9f4c9835d8018ff7dbdb5683/c06f405c6cafaa13845f6727/f937aebd788e8d724a4de67b", version : "c6a0a90fd8181defba24bed3");//numberUtils.fs
...
export const WIDTH_VARIATION_BOUNDS =
{
            (inch) : [0, 0.1, 1e6]
        } as LengthBoundSpec;

// Random seed constants for UI constraint and LCG remapping
const RANDOM_SEED_UI_MAX = 10000000;
const LCG_MODULUS = 2147483648; // 2^31, matches pseudoRandomNumber implementation

annotation { "Feature Type Name" : "OnlyTabs", "Icon" : ICON::BLOB_DATA, "Feature Type Description" : "Tab and Slot feature for Boss Display", "Editing Logic Function" : "tabAndSlotEditingLogic" }
export const tabAndSlotBossDisplay = defineSheetMetalFeature(function(context is Context, id is Id, definition is map)
    precondition
    {
        ...
        annotation { "Group Name" : "Tab width randomization", "Collapsed By Default" : true }
        {
            ...
            {
                annotation { "Name" : "Random seed", "Default" : 1000, "UIHint" : UIHint.REMEMBER_PREVIOUS_VALUE }
                isInteger(definition.randomSeed, { (unitless) : [0, 1000, RANDOM_SEED_UI_MAX] } as IntegerBoundSpec);

                annotation { "Name" : "Width variation", "Description" : "Maximum variation in tab width (+ or -)", "UIHint" : UIHint.REMEMBER_PREVIOUS_VALUE }
                isLength(definition.widthVariation, WIDTH_VARIATION_BOUNDS);

                annotation { "Name" : "Generate new seed" }
                isButton(definition.generateNewSeed);
            }
        }
...
/**
 * Applies corner break attributes to vertices at tab-to-wall connections.
 * 
 * This function processes vertices tracked from the tab extension edges. After tracking through
 * the extension operation and boolean union, the vertex set includes both:
 * - Vertices at outer tab corners (surrounded by tab faces, no wall faces)
 * - Vertices at tab-to-wall connections (have adjacent wall faces)
 * 
 * Only vertices adjacent to wall faces receive corner break attributes. Vertices at outer tab
 * corners are automatically skipped because they have no adjacent wall faces.
 * 
 * @param context - The evaluation context
 * @param id - Operation ID for this corner break application
 * @param definition - Feature definition containing:
 *                     - extensionVerticesTracking: tracked vertices from tab segment edges
 *                     - tabChamfer: the chamfer width for corner breaks
 * @param wallFaces - Query for sheet metal wall faces (unused, kept for signature compatibility)
 */
function applyCornerBreaksToExtensionVertices(context is Context, id is Id, definition is map, wallFaces is Query)
{
    // Early exit if no tracking or zero chamfer
    if (definition.extensionVerticesTracking == undefined || definition.tabChamfer <= 0 * meter)
        return;

    // Resolve tracked vertices after boolean union
    ...
    // Apply corner breaks to vertices adjacent to wall faces
    var attributeCounter = 0;
    for (var vertex in vertexArray)
    {
        try
        {
            // Find adjacent faces with wall attributes
            const adjacentFaces = evaluateQuery(context, qAdjacent(vertex, AdjacencyType.VERTEX, EntityType.FACE));
            
            var wallAttribute = undefined;
            for (var face in adjacentFaces)
            {
                wallAttribute = try silent(getWallAttribute(context, face));
                if (wallAttribute != undefined && wallAttribute.attributeId != undefined)
                    break;
            }

            // Skip vertices not adjacent to walls (outer tab corners)
            if (wallAttribute == undefined || wallAttribute.attributeId == undefined)
                continue;

            const wallId = wallAttribute.attributeId;

            // Check for existing corner attribute
            var existingAttribute = getCornerAttribute(context, vertex);
            var hasExistingAttribute = (existingAttribute != undefined);

            // Skip if corner break already exists for this wall
            ...
            // Create or update corner attribute with chamfer break
 ...
 * @returns : Modified array of tab domains with randomized widths
 */
function applyWidthRandomizationToTabDomains(tabDomains is array, totalLength is ValueWithUnits, definition is map) returns array
{
    ...
    for (var i = 0; i < size(tabDomains); i += 1)
    {
        ...
        const clampedVariation = max(randomVariation, minAllowedWidth - currentTabWidth);
        
        // Calculate new tab width
        const newTabWidth = currentTabWidth + clampedVariation;
        
        // Convert back to normalized parameter space
        const widthDelta = (newTabWidth - currentTabWidth) / totalLength;
        
        // Center the tab: grow/shrink equally on both sides
        const halfDelta = widthDelta / 2;
        
        // Create new domain with adjusted width (centered on original position)
        const newDomain = {
            "start" : domain.start - halfDelta,
            "end" : domain.end + halfDelta
        };
        
        randomizedDomains = append(randomizedDomains, newDomain);
    }
...
/**
 * Editing logic function for tab and slot feature.
 * Handles the "Generate new seed" button click to create a new random seed.
 */
export function tabAndSlotEditingLogic(context is Context, id is Id, oldDefinition is map, definition is map,
    isCreating is boolean, specifiedParameters is map, hiddenBodies is Query, clickedButton is string) returns map
{
    ...
    if (clickedButton == "generateNewSeed")
    {
        // Generate new random seed from current value (or default 1000)
        const currentSeed = definition.randomSeed != undefined ? definition.randomSeed : 1000;
        const lcgOutput = pseudoRandomNumber(currentSeed);
        
        // Map LCG output [0, LCG_MODULUS) to UI range [0, RANDOM_SEED_UI_MAX]
        // Uses same remapping pattern as pseudoRandomNumber in numberUtils.fs
