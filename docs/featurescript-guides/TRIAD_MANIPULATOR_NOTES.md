...
- **editSurface.fs**: The same pattern back-ported onto a B-spline control net, so the live/baked storage, the transposed-rotation convention and the `bakeAgainstPreviousState` split are all shared with FFD rather than reinvented. Two things are specific to a surface and worth reading `docs/specs/EDIT_SURFACE_SPEC.md` §3.1 for. **The base coordinate system is derived, not given**: a lattice already has orthogonal axes, where a control net has to borrow the *surface's* frame at the selection's central control point — u tangent for X and the normal for Z, which is a legal `coordSystem` because `normalize(cross(uTangent, vTangent))` is perpendicular to the u tangent exactly, with a world-axis fallback where the surface is degenerate. **The bake triggers are different**, because what re-indexes the handles is different: approximation, elevation and refinement change *which control points exist*, so each is a trigger, as is a switch to an edit mode that does not write the transform.
...
The split that works: take from `oldDefinition` only what the *interpretation* depends on (the selection, the geometry a stored transform was measured against), and take the *stored values themselves* from the current definition. `routingCurve.fs` sidesteps this entirely by only ever mutating `definition` in place and never assigning an array parameter across from `oldDefinition`; `freeFormDeformation.fs` and `editSurface.fs` cannot, because a bake genuinely has to be interpreted against the old state, and both hold the split in one function (`bakeAgainstPreviousState`) rather than spreading it through the editing logic.
