...
- [ONSHAPE_MCP_USAGE.md](featurescript-guides/ONSHAPE_MCP_USAGE.md) — the Onshape FeatureScript MCP connection and the rules for using it: mirror-first, MCP only for targeted live tests, never for reference lookups.
...
- [SOLID_SWEEP_SPEC.md](specs/SOLID_SWEEP_SPEC.md) — generalized solid sweep: sweep a solid tool body along a path and emit the swept volume's true envelope boundary (Adsul–Machchhar–Sohoni framework adapted to FeatureScript: B-spline motion, a coefficient-certificate topology census, funnel Newton solver, certified B-spline fits). Build steps 1-7 live-validated across the sw*.fs modules; no solid body emitted yet (caps/knit, spec §9, is the next gap). Probes in `custom-features/swSweepProbes.fs`.
